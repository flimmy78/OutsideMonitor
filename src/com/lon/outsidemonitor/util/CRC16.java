...
public class CRC16 {
	...
	private final static int[] CrcTable = new int[] { 0x0000, 0x1021, 0x2042,
	...
	public static int computeCRC16(byte[] buffer, int offset, int count) {
		int crcValue = 0;
		int tempData;
		for (int i = offset; i < count + offset; i++) {
			tempData = (int) (crcValue / 256); // 以8位二进制数的形式暂存CRC的高8位
			crcValue <<= 8; // 左移8位，相当于CRC的低8位乘以 256
			crcValue&=0xffff;
			int index= tempData ^ (buffer[i]&0xff);
		
			crcValue ^= CrcTable[index];// 高8位和当前字节相加后再查表求CRC
														// ，再加上以前的 CRC
		}
		return crcValue;
	}
